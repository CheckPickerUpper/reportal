...
The default config created on first `rep` invocation ships with aider pre-registered.
...
`rjw backend` cd's into `~/dev/workspaces/backend/`, so `ls`, `cargo test`, your IDE — everything sees every member repo as a subdirectory. The symlinks are transparent: `git`, editors, and build tools all resolve them to the real repo.
